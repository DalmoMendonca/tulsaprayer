...
function deterministicModeration(text) {
  ...
  const hardRejectPatterns = [
    ...
    { pattern: /\b(ugly|sucks?|trash|garbage|stupid|awful|terrible|hate|disgusting|worst)\b/, reason: "Please write this as a prayer, not an insult or complaint." },
    { pattern: /\b(i\s+do\s+not|i\s+don't|dont|don't)\s+like\b/, reason: "Please write this as a prayer, not an insult or complaint." },
...
function finalPrayerIntentGate(text) {
  ...
  return { allowed: false, reason: "Please write this as a prayer before posting." };
}
